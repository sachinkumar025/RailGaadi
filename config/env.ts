export const env = {
  // RailRadar — server-side only
  RAILRADAR_API_KEY: process.env.RAILRADAR_API_KEY || '',
  RAILRADAR_BASE_URL: 'https://api.railradar.in/v1',

  // MapTiler — public (safe to expose)
  MAPTILER_API_KEY: process.env.NEXT_PUBLIC_MAPTILER_API_KEY || process.env.MAPTILER_API_KEY || '',

  // OpenWeather — server-side only
  OPENWEATHER_API_KEY: process.env.OPENWEATHER_API_KEY || '',

  // OpenTopography — server-side only
  OPENTOPOGRAPHY_API_KEY: process.env.OPENTOPOGRAPHY_API_KEY || '',

  // Upstash Redis (Phase 2)
  UPSTASH_REDIS_REST_URL: process.env.UPSTASH_REDIS_REST_URL || '',
  UPSTASH_REDIS_REST_TOKEN: process.env.UPSTASH_REDIS_REST_TOKEN || '',
};

// Client-safe config — only include NEXT_PUBLIC_ variables here
export const clientEnv = {
  MAPTILER_API_KEY: process.env.NEXT_PUBLIC_MAPTILER_API_KEY || process.env.MAPTILER_API_KEY || '',
};

// Validation helpers
export function isRailRadarConfigured(): boolean {
  return Boolean(env.RAILRADAR_API_KEY && env.RAILRADAR_API_KEY.trim() !== '');
}

export function isMapTilerConfigured(): boolean {
  return Boolean(clientEnv.MAPTILER_API_KEY && clientEnv.MAPTILER_API_KEY.trim() !== '');
}

export function isOpenWeatherConfigured(): boolean {
  return Boolean(env.OPENWEATHER_API_KEY && env.OPENWEATHER_API_KEY.trim() !== '');
}

export function isOpenTopographyConfigured(): boolean {
  return Boolean(env.OPENTOPOGRAPHY_API_KEY && env.OPENTOPOGRAPHY_API_KEY.trim() !== '');
}

